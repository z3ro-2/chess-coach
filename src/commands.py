...
def _status_command(conn: sqlite3.Connection, args: Any) -> CommandResult:
    ...
    lines = [
        ...
        f"Pending: {int(queue_diag.get('pending_total', 0) or 0)}",
        f"Eligible: {int(queue_diag.get('eligible_now', 0) or 0)}",
        f"Cooldown-blocked: {int(queue_diag.get('excluded_by_cooldown', 0) or 0)}",
        f"Attempt-cap-blocked: {int(queue_diag.get('excluded_by_attempt_cap', 0) or 0)}",
        "Queue Health:",
