...
def test_ensure_postgres_core_schema_includes_failure_notified_column(monkeypatch) -> None:
    ...
    def _table_columns_stub(_conn, table_name: str):
        if table_name == "players":
            return {"id", "username"}
        if table_name == "games":
            return {"id", "player_id", "failure_notified"}
        return set()
    ...
    assert any("CREATE TABLE IF NOT EXISTS games" in q and "failure_notified BOOLEAN NOT NULL DEFAULT FALSE" in q for q in executed)
